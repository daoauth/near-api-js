...
class ExtensionWalletSigner extends Signer {
    ...
    async getPublicKey() {
        if (window.dapp) {
            const response = await window.dapp.request({ net: 'near:*', method: 'dapp:accounts' });
            if (response && response.near && response.near.length > 0) {
                return key_pair_1.PublicKey.fromString(response.near[0]);
            }
        }
        return null;
    }
    async signMessage(message, accountId, networkId) {
        if (window.dapp) {
            ...
            const [signature, publicKey] = await window.dapp.request({ net: `near:${networkId || '*'}`, method: 'dapp:sign', params: accountId ? [hex, accountId] : [hex] });
