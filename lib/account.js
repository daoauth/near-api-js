...
class Account {
    ...
    async signAndSendTransactionV2({ receiverId, actions, returnError }) {
        ...
        const result = await exponential_backoff_1.default(TX_NONCE_RETRY_WAIT, TX_NONCE_RETRY_NUMBER, TX_NONCE_RETRY_WAIT_BACKOFF, async () => {
            if (!this.connection.provider.signAndSendTransaction) {
                ...
                const publicKey = this.connection.provider.pubKey();
                const accessKeyInfo = await this.findAccessKey(receiverId, actions);
                if (!accessKeyInfo) {
                    throw new providers_1.TypedError(`Can not sign transactions for account ${this.accountId} on network ${this.connection.networkId}, no matching key pair found in ${this.connection.signer}.`, 'KeyNotFound');
                }
                ...
                const blockHash = block.header.hash;
                const nonce = ++accessKey.nonce;
                const transaction = common_index_1.transactions.createTransaction(this.accountId, key_pair_1.PublicKey.fromString(publicKey), receiverId, nonce, actions, borsh_1.baseDecode(blockHash));
                return this.connection.provider.signAndSendTransaction(transaction);
            }
        });
        if (!result) {
