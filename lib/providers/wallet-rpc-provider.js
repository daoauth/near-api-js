...
exports.WalletRpcProvider = exports.ErrorContext = exports.TypedError = void 0;
/**
 ...
 */
class WalletRpcProvider extends provider_1.Provider {
    constructor() {
        super();
        this._network = '';
        this._account = '';
        this._pubKey = '';
        if (window && window.dapp && window.dapp.networks.near) {
        ...
        const response = await exponential_backoff_1.default(REQUEST_RETRY_WAIT, REQUEST_RETRY_NUMBER, REQUEST_RETRY_WAIT_BACKOFF, async () => {
            try {
                ...
                if (method === 'tx') {
                    ...
                    const response = await web_1.fetchJson({ url }, JSON.stringify(request));
                    console.log('----');
                ...
                if (typeof txHash !== 'string' || txHash.error) {
                    const error = txHash.error;
                    if (typeof error.data === 'object') {
                        if (typeof error.data.error_message === 'string' && typeof error.data.error_type === 'string') {
                            // if error data has error_message and error_type properties, we consider that node returned an error in the old format
                            throw new errors_1.TypedError(error.data.error_message, error.data.error_type);
                        }
                        throw rpc_errors_1.parseRpcError(error.data);
                    }
                    else {
                        ...
                        throw new errors_1.TypedError(errorMessage, rpc_errors_1.getErrorTypeFromErrorMessage(error.data));
                    }
                }
                return txHash;
            }
